...
document.addEventListener("add-note", (event) => {
    ...
    addNote(title, body); // Pakai fungsi addNote yang sudah ada
});
...
function renderNotes() {
    ...
    notesData.forEach(note => {
        ...
        noteElement.setAttribute("data-id", note.id); // Simpan ID sebagai atribut dataset
...
function searchNotes() {
    ...
    const filteredNotes = notesData.filter(note => note.title.toLowerCase().includes(searchValue));
